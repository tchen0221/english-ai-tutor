...
def generate_module_3(vocab_data):
    ...
    def _generate_cloze():
        ...
        【核心要求】
        1. 主题指定：人际交往 / 校园生活 / 克服个人困难的记叙小故事。（⚠️ 绝对禁止写自然环保或灾害主题，以免与另一题冲突）
        2. 篇幅150-180词，完整故事情节（起因-冲突-解决-感悟）。首句必须完整，严禁挖空。
        3. 挖空 5 处（编号 1 到 5），标记为 ___1___。
        4. ⚠️ 科学与逻辑约束：故事情节必须完全符合现实常识（绝不允许出现“干旱导致海平面下降”等缺乏科学常识的病句）。
        5. 上下文强锁死：单看所在句应有多个选项通顺，但结合上下文情节，【必须有且仅有 1 个】合乎逻辑的答案。
        6. 严禁明文泄题，正确答案原词不可在上下文中出现。四个选项词性必须严格一致。
    ...
    def _generate_short_text():
        ...
        【核心要求】
        1. 主题指定：自然环保 / 科学常识 / 科技与社会发展。（⚠️ 绝对禁止写人际交往或校园故事，以免与另一题冲突）
        2. 篇幅130-160词。挖空必须均匀分布，严禁连续两句挖空。
        3. 严格挖空 5 处（编号 6 到 10）。
        4. ⚠️ 致命编号规则：文章中的挖空标记必须且只能在文本中【严格按从小到大递增的顺序】出现！即文本从前到后必须依次是 ___6___, ___7___, ___8___, ___9___, ___10___，绝对禁止乱序（如 7 跑到 6 前面这种错误绝对不能发生）！
        5. 考点公式硬性要求（必须严格遵守！）：
